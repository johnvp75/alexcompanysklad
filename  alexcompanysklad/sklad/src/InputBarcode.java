...
//		Новый товар
		...
		try {
			if (cod.charAt(0)=='*')
				...
				if (cod.charAt(0)=='-'){
					SQL=String.format("select trim(name), 1 as count from tovar where id_tovar in (select distinct id_tovar from bar_code where lower(trim(bar_code)) like '%s%s' and id_skl=(select id_skl from SKLAD where name='%s')) order by tovar.name","%",cod.substring(1).toLowerCase(),sklad);
					rs=DataSet.QueryExec(SQL, false);
				}else
