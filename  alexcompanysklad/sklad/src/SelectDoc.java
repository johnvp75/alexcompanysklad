...
public class SelectDoc extends MyPanel {
	...
	public SelectDoc(int Type_doc, boolean Register) {
		...
		String SQL="select trim(client.name), document.sum, trim(val.name), trim(sklad.name), trim (document.note) from ((document inner join client on document.id_client=client.id_client) " +
